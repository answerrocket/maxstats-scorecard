...
@skill(
    name="databricks_scorecard",
    description="KPI Scorecard for Max Product Performance showing user activity metrics (Databricks-compatible SQL)",
    parameters=[
        SkillParameter(
            name="tenants",
            is_multi=True,
...
def databricks_scorecard(parameters: SkillInput) -> SkillOutput:
...
def get_users_stats(tenants, dataset_metadata):
    ...
    Databricks/Spark SQL does not support referencing SELECT-list aliases
    (e.g. `tenant`) in the WHERE clause the way Redshift does, and uses
    different syntax for date casts and interval literals. The query below
    is a Databricks-compatible rewrite of the Redshift version in
    `scorecard.py`:
      - The computed `tenant` value is materialized in a `base` CTE so it
        can be referenced in the outer WHERE.
      - `<col>::date` casts are replaced with `CAST(<col> AS DATE)`.
      - `INTERVAL '<n> days'` literals are replaced with `INTERVAL <n> DAYS`.
    """
    ...
    WITH base AS (
        SELECT
            ...
            END AS tenant
        FROM {dataset_source}
        WHERE asked_at_timestamp IS NOT NULL
    ...
    SELECT
        ...
        CASE WHEN CAST(asked_at_timestamp AS DATE) >= CURRENT_DATE - INTERVAL 1 DAY
             AND  CAST(asked_at_timestamp AS DATE) <  CURRENT_DATE THEN 1 ELSE 0 END AS new_user_yesterday,
        MIN(asked_at_timestamp) OVER (PARTITION BY user_name) AS user_first_seen
    FROM base
    WHERE LOWER(tenant) IN ({tenant_filters})
    GROUP BY user_name, asked_at_timestamp, tenant
    """
...
if __name__ == '__main__':
    skill_input: SkillInput = databricks_scorecard.create_input(
        arguments={'tenants': ['cpw prod']}
    )
    out = databricks_scorecard(skill_input)
    preview_skill(databricks_scorecard, out)
